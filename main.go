...
func main() {
	...
	s3Config, s3Error := config.LoadDefaultConfig(context.Background(), config.WithRegion(s3Region))
	if s3Error != nil {
		log.Fatal("Unable to connect to s3")
	}
